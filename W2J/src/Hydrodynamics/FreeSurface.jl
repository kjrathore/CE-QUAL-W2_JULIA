...
# session so far. The goal is a runnable first cut producing real TSR output
# for Detroit under a zero-boundary-flow sanity check, not full fidelity --
# parallel processing is the explicitly agreed next phase after this, not
# more physics fidelity first. What's real vs. stubbed:
#
...
solve, branch-sequenced via `Core/Grid.jl`'s `branch_processing_order` (a
REAL requirement, not a nicety -- see that function's docstring for the
traced proof this is a genuine sequential dependency, not parallel like the
TKE/momentum TRIDIAG). Updates `geom.Z` and `geom.ELWS`.
...
function solve_free_surface!(g, geom, net, dlt)
    ...
    for jw in 1:g.NWB
        kt = g.KTWB[jw]
        for jb in branch_processing_order(g, net, jw)
